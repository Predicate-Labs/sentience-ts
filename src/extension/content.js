// Sentience Chrome Extension - Content Script
...
(function () {
  ...
  function handleScreenshotRequest(data) {
    chrome.runtime.sendMessage({ action: 'captureScreenshot', options: data.options }, (response) => {
      window.postMessage(
        {
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          ...
          if (chrome.runtime.lastError) {
            console.error(
              '[Sentience Bridge] Chrome runtime error:',
  ...
  function handleShowOverlay(data) {
    ...
    elements.forEach((element) => {
      ...
      const borderOpacity = isTarget
        ? 1.0
        : isPrimary
          ? 0.9
          : Math.max(0.4, 0.5 + importanceRatio * 0.5);
      const fillOpacity = borderOpacity * 0.2;
      const borderWidth = isTarget
        ? 2
        : isPrimary
          ? 1.5
          : Math.max(0.5, Math.round(importanceRatio * 2));
